...
from python_som import SampleMode, WeightInit
...
def test_linear_init_is_deterministic(blobs: np.ndarray) -> None:
    ...
    a.weight_initialization(mode=WeightInit.LINEAR, data=blobs)
    b.weight_initialization(mode=WeightInit.LINEAR, data=blobs)
...
def test_linear_init_rejects_a_degenerate_dataset() -> None:
    ...
    with pytest.raises(ValueError, match="at least 2 samples"):
        som.weight_initialization(mode=WeightInit.LINEAR, data=np.zeros((5, 1)))
...
@pytest.mark.parametrize("sample_mode", list(SampleMode))
def test_random_init_modes(sample_mode: SampleMode) -> None:
    som = make_som(x=5, y=5, input_len=3)
    som.weight_initialization(mode=WeightInit.RANDOM, sample_mode=sample_mode)
...
def test_random_init_rejects_an_unknown_sample_mode() -> None:
    ...
    with pytest.raises(ValueError, match="sample_mode"):
        som.weight_initialization(mode=WeightInit.RANDOM, sample_mode="cauchy")
...
def test_initialization_is_reproducible_from_the_seed() -> None:
    ...
    a.weight_initialization(mode=WeightInit.RANDOM)
    b.weight_initialization(mode=WeightInit.RANDOM)
